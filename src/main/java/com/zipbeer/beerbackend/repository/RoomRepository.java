...
public interface RoomRepository extends JpaRepository<RoomEntity, Long> {
    ...
    @Query("select r from RoomEntity r where r.category like concat('%',:category,'%') and r.title like concat('%',:searchTerm,'%')")
    Page<RoomEntity> findAllByTitle(Pageable pageable, @Param("searchTerm") String searchTerm, @Param("category") String category);

    @Query("select r from RoomEntity r where r.category like concat('%',:category,'%') and r.master like concat('%',:searchTerm,'%')")
